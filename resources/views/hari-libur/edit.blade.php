<x-app-layout>
    ...
    <x-slot name="subtle">Perbarui data hari libur atau hari khusus</x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 w-full">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h2 class="text-xl font-bold text-gray-800">Edit Hari Libur / Hari Khusus</h2>
            </div>

            <form action="{{ route('hari-libur.update', $hariLibur) }}" method="POST" class="space-y-6" x-data="{
                tipe: '{{ old('tipe', $hariLibur->tipe ?? 'libur') }}',
                isMasuk: {{ old('is_masuk', $hariLibur->is_masuk) ? 'true' : 'false' }},
                isLembur: {{ old('is_lembur', $hariLibur->is_lembur ?? true) ? 'true' : 'false' }},
                isShiftEnabled: {{ old('is_shift_enabled', $hariLibur->is_shift_enabled) ? 'true' : 'false' }},
                pegawaiMode: '{{ (old('pegawai_semua') || !old('pegawai_hadir', $hariLibur->pegawai_hadir)) ? 'semua' : 'pilih' }}'
            }">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative">
                            <input type="radio" name="tipe" value="libur" x-model="tipe"
                                class="peer sr-only" {{ old('tipe', $hariLibur->tipe ?? 'libur') === 'libur' ? 'checked' : '' }}>
                        ...
                        <label class="relative">
                            <input type="radio" name="tipe" value="hari_khusus" x-model="tipe"
                                class="peer sr-only" {{ old('tipe', $hariLibur->tipe) === 'hari_khusus' ? 'checked' : '' }}>
                            <div class="p-4 rounded-xl border-2 cursor-pointer transition-all
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                        ...
                        class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $hariLibur->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="border-t border-gray-200 pt-6" x-show="tipe === 'hari_khusus'" x-transition>
                    ...
                    <div class="flex items-start gap-3 mb-6">
                        <input type="checkbox" name="is_masuk" id="is_masuk" value="1" x-model="isMasuk"
                            {{ old('is_masuk', $hariLibur->is_masuk) ? 'checked' : '' }}
                            class="mt-1 w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                        <div>
                        ...
                        <div class="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl p-4 border border-yellow-200">
                            <div class="flex items-start gap-3">
                                <input type="checkbox" name="is_lembur" id="is_lembur" value="1" x-model="isLembur"
                                    {{ old('is_lembur', $hariLibur->is_lembur ?? true) ? 'checked' : '' }}
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="time" name="jam_masuk" id="jam_masuk"
                                        value="{{ old('jam_masuk', $hariLibur->jam_masuk ? \Carbon\Carbon::parse($hariLibur->jam_masuk)->format('H:i') : '') }}"
                                ...
                                <div>
                                    ...
                                    <input type="time" name="jam_keluar" id="jam_keluar"
                                        value="{{ old('jam_keluar', $hariLibur->jam_keluar ? \Carbon\Carbon::parse($hariLibur->jam_keluar)->format('H:i') : '') }}"
                                        class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 space-y-4">
                            <div class="flex items-start gap-3">
                                <input type="checkbox" name="is_shift_enabled" id="is_shift_enabled" value="1" x-model="isShiftEnabled"
                                    {{ old('is_shift_enabled', $hariLibur->is_shift_enabled) ? 'checked' : '' }}
                                    class="mt-1 w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="time" name="shift1_jam_masuk"
                                                value="{{ old('shift1_jam_masuk', $hariLibur->shift1_jam_masuk ? \Carbon\Carbon::parse($hariLibur->shift1_jam_masuk)->format('H:i') : '') }}"
                                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-600 mb-2">Jam Keluar</label>
                                            <input type="time" name="shift1_jam_keluar"
                                                value="{{ old('shift1_jam_keluar', $hariLibur->shift1_jam_keluar ? \Carbon\Carbon::parse($hariLibur->shift1_jam_keluar)->format('H:i') : '') }}"
                                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="time" name="shift2_jam_masuk"
                                                value="{{ old('shift2_jam_masuk', $hariLibur->shift2_jam_masuk ? \Carbon\Carbon::parse($hariLibur->shift2_jam_masuk)->format('H:i') : '') }}"
                                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-600 mb-2">Jam Keluar</label>
                                            <input type="time" name="shift2_jam_keluar"
                                                value="{{ old('shift2_jam_keluar', $hariLibur->shift2_jam_keluar ? \Carbon\Carbon::parse($hariLibur->shift2_jam_keluar)->format('H:i') : '') }}"
                                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">
                                        </div>
                                    </div>
                                </div>
                                ...
                                @foreach($pegawai as $p)
                                    <label class="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-primary/50 cursor-pointer transition-all">
                                        <input type="checkbox" name="pegawai_hadir[]" value="{{ $p->id }}"
                                            {{ in_array($p->id, $selectedPegawai) ? 'checked' : '' }}
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 space-y-4">
                            ...
                            <div class="flex items-start gap-3">
                                <input type="checkbox" name="libur_tetap_masuk" id="libur_tetap_masuk" value="1"
                                    {{ old('libur_tetap_masuk', $hariLibur->libur_tetap_masuk) ? 'checked' : '' }}
                                    class="mt-1 w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                                <div>
                            ...
                            <div class="flex items-start gap-3">
                                <input type="checkbox" name="is_wajib" id="is_wajib" value="1"
                                    {{ old('is_wajib', $hariLibur->is_wajib) ? 'checked' : '' }}
                                    class="mt-1 w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                                <div>
